...
export default function LogsAdminPage() {
    ...
    const size = 20;
    const { data, mutate: mutateLogs, isLoading } = useSWR("/api/admin/logs?page=" + page + "&size=" + size, fetcher, { refreshInterval: 5000 });
    ...
    const levelColors = {
        ...
        SECURITY: "text-purple-400 font-black tracking-widest outline outline-1 outline-purple-500/30 px-1 rounded animate-pulse mr-4",
    }
        ...
        <div className="flex flex-col gap-6 h-[calc(100vh-280px)]">
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-bold flex items-center gap-2">
                    <TerminalIcon className="size-5 text-primary" /> Operational Audit Trail
                </h2>
            ...
            <div className="flex items-center gap-4 bg-muted/20 p-2 rounded-2xl border border-border/40">
                ...
                <div className="flex items-center gap-2 px-3 border-l border-border/40">
                    <span className="text-[10px] font-mono text-muted-foreground uppercase">{page}/{totalPages}</span>
                    <div className="flex gap-1">
                        <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="disabled:opacity-20"><ChevronLeftIcon className="size-3" /></button>
                        <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page >= totalPages} className="disabled:opacity-20"><ChevronRightIcon className="size-3" /></button>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 rounded-2xl border border-primary/20 overflow-hidden flex flex-col shadow-2xl">
                <div className="flex items-center gap-2 px-4 py-2 bg-muted/10 border-b border-border/10">
                    ...
                    <div className="mx-auto text-[10px] font-mono text-muted-foreground/50 tracking-widest uppercase">
                        HacxGPT_CLI_BUFFER v1.0.4 <span className="ml-2 opacity-80">| TOTAL_RECORDS: {total}</span>
                    </div>
                </div>
                <div
                    ref={scrollRef}
                ...
                >
                    {filteredLogs.map((log: LogEntry, i: number) => (
                        <div key={i} className="flex gap-4 group hover:bg-white/5 py-0.5 px-1 rounded transition-colors">
                            <span className="text-muted-foreground/40 shrink-0 select-none">[{new Date(log.timestamp).toLocaleTimeString()}]</span>
                            <span className={`font-bold shrink-0 w-12 ${levelColors[log.level] || 'text-muted-foreground'}`}>{log.level}</span>
                            <span className="text-primary/70 shrink-0 w-16">[{log.module}]</span>
                            <span className="text-foreground/80">{log.message}</span>
                        </div>
                    ))}
                    {!filteredLogs.length && !isLoading && (
                        <div className="text-muted-foreground/30 py-4 italic">End of stream or no matches on this page.</div>
                    )}
